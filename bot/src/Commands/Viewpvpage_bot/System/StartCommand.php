    ...
    public function execute()
    {
		...
		if ($message && $chat_type == 'private') {
			...
			if ($user_status == 'member' || $user_status == 'creator' || $user_status == 'administrator'){
				...
				$text = 'با ربات' .'  View Private Page  ' .'میتونی پست های پیج های خصوصی اینستگرام را مشاهده کنید.' .json_decode('"'.'\uD83D\uDE0E' .'"') .PHP_EOL 
			...
			$text = 'با ربات' .'  View Private Page  ' .'میتونی پست های پیج های خصوصی اینستگرام را مشاهده کنید.' .json_decode('"'.'\uD83D\uDE0E' .'"') .PHP_EOL 
				...
				.'t.me/Viewpvpage_bot?start=' .$text_m .PHP_EOL .PHP_EOL;
		}
        $data = [
            'chat_id'      => $chat_id,
            'text'         => $text,
		...
		if ($message && $chat_type == 'private') {
			...
			$keyboard->setResizeKeyboard(true)
					 ...
					 ->setSelective(false);
			$text = 'بعد برای ادامه کار دکمه ارسال ایدی پیج برای مشاهده یا دستور' .PHP_EOL
					.'/sendpage' .PHP_EOL
					.'رو بزن';
			$data = [
				'chat_id'      => $chat_id,
