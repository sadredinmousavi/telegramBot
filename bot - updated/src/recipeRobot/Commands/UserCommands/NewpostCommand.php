    ...
    switch ($state) {
        ...
        case 0:
            ...
            if (in_array($type, ['audio', 'document', 'photo', 'video', 'voice'], true)) {
    			...
    			// }
                $doc_array = (array) $doc;
                unset($doc_array['raw_data']);
                unset($doc_array['bot_username']);
                $notes['doc'] = $doc_array;
                //echo "<pre> ", $type, "\n", print_r($notes[$type]), "</pre>";
    		} else {
                $support = 0;
            }
            if ($support === 0 || ($type === 'document' && !in_array($notes['doc']['mime_type'], ['application/pdf', 'application/zip', 'video/mp4'], true))){
                $data['chat_id'] = $chat_id;
                $data['text'] = 'این نوع فایل ساپورت نمی شود.';
                $result = Request::sendMessage($data);
            ...
            $is_error = 0;
            if ($result->isOK()){
                $notes['link'] = 'https://t.me/' .$channel_upload .'/' .$result->getResult()->getmessage_id();
                $m_id = $result->getResult()->getmessage_id();
                // $doc = call_user_func([$result->getResult(), 'get' . ucfirst($type)]);
                // echo "<pre>", print_r($result), print_r($doc), "</pre>";
                if (isset($notes['doc']['thumb']['file_id'])){
                    $data =[];
                    $data['chat_id'] = $channel_upload_id;
                    $data['document']= trim($notes['doc']['thumb']['file_id']);
                    $result = Request::sendDocument($data);
                    if ($result->isOK()){
                        $notes['doc']['thumb']['link'] = 'https://t.me/' .$channel_upload .'/' .$result->getResult()->getmessage_id();
                    } else {
                        // $is_error = 1;
                        $notes['doc']['thumb']['link'] = 'https://t.me/zasxwq/73';
                    }
                    //echo "<pre>", print_r($result), print_r($notes['doc']), print_r($data), "</pre>";
                }
            } else {
                $is_error = 1;
            }
