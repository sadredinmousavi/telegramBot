...
class InstagramHelper
{
    ...
    public $endpoint = array(
        ...
        'media_json'                      => 'https://www.instagram.com/{user}/media/?__a=1',
        'media_json_next_call'            => 'https://www.instagram.com/{user}/media/?__a=1&max_id={max_id}',
        ...
        'search_related_photos_json'      => 'https://www.instagram.com/p/{code}/?__a=1',
    );
    /**
    ...
    protected $default_request_action = array(
        'get_user_info', 'search_info','get_user_last_media', 'top_search','get_media_details'
    );
    /**
     * The request action
     * @var string
     */
    public $request_action = 'get_user_info';
    /**
     ...
     */
    public function initialize($request_action = 'get_user_info', $request_data = array())
    {
        // validate request
    ...
    public function setInstagramRequest()
    {
        if (1) {
            ...
            } elseif ($this->request_action === 'top_search'){
                $this->users_input = str_replace('{keyword}', $this->users_input[$this->input_key], $this->endpoint['search_all_tags_json']);
                if ($this->sendInstagramRequest() && $this->buildResultFromCurl()) {
                    // return response
                    return $this->getInstagramResponse();
                }
            } elseif ($this->request_action === 'get_user_last_media'){
                if (isset($this->users_input['max_id'])) {
                    $this->users_input = str_replace(array('{user}', '{max_id}'), array($this->users_input[$this->input_key], $this->users_input['max_id']), $this->endpoint['media_json_next_call']);
                    // tasiri nadarad
                } else {
                    $this->users_input = str_replace('{user}', $this->users_input[$this->input_key], $this->endpoint['media_json']);
                }
                if ($this->sendInstagramRequest() && $this->buildResultFromCurl()) {
                    // return response
                    return $this->getInstagramResponse();
                }
            } elseif ($this->request_action === 'get_media_details'){
                $this->users_input = str_replace('{code}', $this->users_input[$this->input_key], $this->endpoint['search_related_photos_json']);
                if ($this->sendInstagramRequest() && $this->buildResultFromCurl()) {
                    // return response
                    return $this->getInstagramResponse();
                }
    ...
    private function buildResultFromCurl()
    {
        ...
        print_r(json_decode(preg_replace($this->regex_pattern['object'], '', $output[2]), true));
    ...
    public function run($request, $params)
    {
        ...
        $result_raw = $this->setInstagramRequest();
        //
